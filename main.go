...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	meanSpeedKmh := meanSpeed(action, duration) // Вычисление средней скорости бега в км/ч

	// Вычисляем потерянные калории при беге по формуле
	calories := (runningCaloriesMeanSpeedMultiplier * meanSpeedKmh * runningCaloriesMeanSpeedShift * weight / mInKm) * duration * minInH
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	// Вычисляем потерянные калории при плавании по формуле
	calories := (meanSpeedKmh + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return calories
}
